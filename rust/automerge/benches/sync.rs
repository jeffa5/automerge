...
fn sync(doc1: &mut DocWithSync, doc2: &mut DocWithSync) {
    while let Some(message1) = doc1.doc.generate_sync_message(&mut doc1.peer_state, usize::MAX) {
        doc2.doc
            .receive_sync_message(&mut doc2.peer_state, message1)
            .unwrap();

        if let Some(message2) = doc2.doc.generate_sync_message(&mut doc2.peer_state, usize::MAX) {
            doc1.doc
